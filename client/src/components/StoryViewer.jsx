   ...
   const handleClose=()=>{
    ...
    const renderContent =()=>{
      switch(viewStory.media_type){
        case 'image':
              return(
                  <img src={viewStory.media_url} alt=""  className='max-w-full max-h-screem object-contain'/>
              );
         case 'video':
              return(
                  <video onEnded={()=> setViewStory(null)} src={viewStory.media_url} className=' max-h-[90vh] max-w-full object-contain ' controls autoPlay />
              );
           case 'text':
              return(
  ...
  return (
    <div className='fixed inset-0 h-screen bg-black bg-opacity-90 z-110 flex items-center justify-center' style={{backgroundColor:viewStory.media_type ==='text' ? viewStory.background_color :'#000000'   }}>
      ...
      <div className='absolute top-4 left-4 flex items-center space-x-3 p-2 px-4 sm:p-4 sm:-px-8 backdrop-blur-2xl rounded bg-black/50'>
          ...
          <div className='text-white font-medum flex items-center gap-1.5'>
        ...
        {/* Content Wrapper */}
        <div className='max-w-[90vw] max-h-[90vh] flex items-center justify-centerstify'>
              {renderContent()}
        </div>
    </div>
  )
