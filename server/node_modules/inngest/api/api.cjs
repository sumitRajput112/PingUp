...
const checkpointNewRunResponseSchema = zod_v3.z.object({ data: zod_v3.z.object({
	...
	realtime_token: zod_v3.z.string().min(1)
}) });
var InngestApi = class {
	...
	get apiHeaders() {
		...
		return {
			[require_consts.headerKeys.SdkVersion]: `inngest-js:v${require_version.version}`,
			...environment ? { [require_consts.headerKeys.Environment]: environment } : {}
		};
	}
	get hashedKey() {
	...
	*/
	async checkpointStream(args) {
		...
		const res = await require_net.fetchWithAuthFallback({
			authToken: this.hashedKey,
			authTokenFallback: this.hashedFallbackKey,
			fetch: this._fetch(),
			url,
