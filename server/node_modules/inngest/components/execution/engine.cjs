...
const require_metadataProcessor = require('./otel/metadataProcessor.cjs');
...
var InngestExecutionEngine = class extends require_InngestExecution.InngestExecution {
			...
			this.execution = require_als.getAsyncLocalStorage().then((als) => {
				return als.run({
					...
					return tracer.startActiveSpan("inngest.execution", (span) => {
						let aiMetadataScope;
						if (this.options.client.aiMetadataEnabled) aiMetadataScope = require_metadataProcessor.metadataSpanProcessor.startScope({
						...
						if (aiMetadataScope) require_metadataProcessor.metadataSpanProcessor.declareStartingSpan({
							scope: aiMetadataScope,
							span,
							traceparent: this.options.headers[require_consts.headerKeys.TraceParent],
							onAIMetadata: (aiMetadata) => {
								if (!this.state.executingStep) return;
								this.state.executingStepAIMetadata = this.state.executingStepAIMetadata ? require_aiExtractor.aggregate(this.state.executingStepAIMetadata, aiMetadata) : aiMetadata;
							}
						});
						return this._start(() => {
							span.end();
							if (aiMetadataScope) require_metadataProcessor.metadataSpanProcessor.endScope(aiMetadataScope);
						}).then((result) => {
	...
	*/
	async _start(onCoreLoopSettled) {
	...
	async checkpoint(steps) {
		...
		if (this.options.stepMode === require_types.StepMode.Sync) if (!this.state.checkpointedRun) {
			...
			await require_promises.retryWithBackoff(() => this.options.client["inngestApi"].checkpointStepsAsync({
				...
				...CHECKPOINT_RETRY_OPTIONS,
				shouldRetry: (err) => !require_marker.isStaleDispatchError(err)
			});
	...
	*/
	createCheckpointHandlers() {
		...
		const maybeReturnNewSteps = async (foundSteps = Array.from(this.state.steps.values())) => {
			const allSteps = [...await this.filterNewSteps(foundSteps) ?? [], ...this.state.lazyOps.drain()];
		...
		const attemptCheckpointAndResume = async (stepResult, resume = true, force = false) => {
			...
			if (force || !this.options.checkpointingConfig?.bufferedSteps || this.state.checkpointingStepBuffer.length >= this.options.checkpointingConfig.bufferedSteps) {
				...
				try {
					...
					if (require_marker.isStaleDispatchError(err)) {
						this.devDebug("stale dispatch detected; halting execution");
						return {
							type: "function-rejected",
							ctx: this.fnArg,
							ops: {},
							error: require_errors.serializeError(err),
							retriable: false
						};
					}
		...
		const asyncHandlers = {
			...
			"steps-found": async ({ steps }) => {
				...
				if (!stepResult) return maybeReturnNewSteps(steps);
	...
	*/
	...
	createFnArg() {
		...
		if (this.options.client[require_Inngest.sessionPropagationSymbol]) try {
			const sessions = require_sessions.reduceEventsToPropagatedSessions(fnArg.events ?? []);
			if (Object.keys(sessions).length > 0) fnArg.sessions = sessions;
		} catch {}
		/**
	...
	createStepTools() {
		...
		*/
		...
		const stepHandler = async ({ args, matchOp, opts }) => {
			...
			if (opId.op === require_types.StepOpCode.InvokeFunction && this.options.client[require_Inngest.sessionPropagationSymbol] && opId.opts?.payload) opId.opts.payload = require_sessions.stampPropagatedSessionsOnEvent(opId.opts.payload, this.fnArg.sessions);
 ...
 /**
			...
			*/
			...
			const asyncContext = require_als.getAsyncCtxSync();
			const step = {
				...opId,
				opts: {
					...opId.opts,
				...
				handle: () => {
					...
					if (!asyncContext) return runHandle();
					return require_als.runWithAsyncCtx(asyncContext, runHandle);
				}
			};
	...
	buildDefer(stepHandler) {
		return (idOrOptions, { function: deferFn, data, experiment, meta }) => {
			...
			try {
				...
				const finalInput = experiment && require_types$1.isRecord(input) ? {
					...input,
					[require_consts.deferExperimentKey]: experiment
				} : input;
				let deferMeta;
				try {
					deferMeta = require_sessions.normalizeEventMeta(meta);
					if (this.options.client[require_Inngest.sessionPropagationSymbol]) {
						const propagated = require_sessions.normalizePropagatedSessions(this.fnArg.sessions);
						if (propagated) deferMeta = {
							...deferMeta,
