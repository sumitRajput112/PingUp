...
import { isDeferredFunction, isStaleDispatchError } from "../../helpers/marker.js";
...
import { getAsyncCtx, getAsyncCtxSync, getAsyncLocalStorage, runWithAsyncCtx } from "./als.js";
import { normalizeEventMeta, normalizePropagatedSessions, reduceEventsToPropagatedSessions, stampPropagatedSessionsOnEvent } from "../../helpers/sessions.js";
...
import { internalLoggerSymbol, sessionPropagationSymbol } from "../Inngest.js";
...
var InngestExecutionEngine = class extends InngestExecution {
			...
			this.execution = getAsyncLocalStorage().then((als) => {
				return als.run({
					...
					return tracer.startActiveSpan("inngest.execution", (span) => {
						let aiMetadataScope;
						if (this.options.client.aiMetadataEnabled) aiMetadataScope = metadataSpanProcessor.startScope({
						...
						if (aiMetadataScope) metadataSpanProcessor.declareStartingSpan({
						...
						return this._start(() => {
							span.end();
							if (aiMetadataScope) metadataSpanProcessor.endScope(aiMetadataScope);
						}).then((result) => {
	...
	*/
	...
	async executeStep(foundStep) {
		...
		if (this.rootSpanId) clientProcessorMap.get(this.options.client)?.declareStepExecution(this.rootSpanId, userland.id ?? "", userland.index ?? 0, hashedId, this.options.data?.attempt ?? 0);
		...
		return goIntervalTiming(() => wrappedActualHandler()).finally(() => {
			...
			if (this.rootSpanId) clientProcessorMap.get(this.options.client)?.clearStepExecution(this.rootSpanId);
	...
	*/
	...
	createFnArg() {
		...
		if (this.options.client[sessionPropagationSymbol]) try {
	...
	createStepTools() {
		...
		*/
		...
		const stepHandler = async ({ args, matchOp, opts }) => {
			...
			if (opId.op === StepOpCode.InvokeFunction && this.options.client[sessionPropagationSymbol] && opId.opts?.payload) opId.opts.payload = stampPropagatedSessionsOnEvent(opId.opts.payload, this.fnArg.sessions);
 ...
 /**
								...
								if (typeof result.data !== "undefined") {
									if (opId.op === StepOpCode.WaitForEvent && result.data !== null) {
										...
										try {
											...
											this.state.recentlyRejectedStepError = new StepError(opId.id, err);
	...
	buildDefer(stepHandler) {
		return (idOrOptions, { function: deferFn, data, experiment, meta }) => {
			...
			try {
				...
				const finalInput = experiment && isRecord(input) ? {
					...input,
	...
	*/
	...
	resumeStepWithResult(resultOp, resume = true) {
		...
		userlandStep.data = JSON.parse(stringify(undefinedToNull(resultOp.data)));
...
//#endregion
export { _internals, createExecutionEngine, engine_exports };
