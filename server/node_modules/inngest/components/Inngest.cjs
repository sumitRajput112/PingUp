...
const require_sessions = require('../helpers/sessions.cjs');
...
const require_metadataProcessor = require('./execution/otel/metadataProcessor.cjs');
...
const require_client = require('./sandbox/client.cjs');
...
const require_sdkFeatureObservations = require('./sdkFeatureObservations.cjs');
...
* @internal
*/
const sessionPropagationSymbol = Symbol.for("inngest.sessionPropagation");
...
var Inngest = class Inngest {
	...
	* @internal
	*/
	[require_sdkFeatureObservations.featureObservationStateSymbol];
	localFns = [];
	/**
	...
	*/
	...
	hasEventApiOriginOverride() {
		return Boolean(this.options.baseUrl || this._env[require_consts.envKeys.InngestEventApiBaseUrl] || this._env[require_consts.envKeys.InngestBaseUrl]);
	}
	[require_sdkFeatureObservations.featureObservationsSymbol]() {
		return require_sdkFeatureObservations.collectFeatureObservations({
			state: this[require_sdkFeatureObservations.featureObservationStateSymbol],
			mode: this.mode,
	...
	*/
	constructor(options) {
		...
		this[require_sdkFeatureObservations.featureObservationStateSymbol] = require_sdkFeatureObservations.sdkFeatureObservations.createState({ aiMetadataEnabled: this.aiMetadataEnabled });
		...
		this.sandboxes = require_client.createSandboxClient({
			baseUrl: () => this.apiBaseUrl,
			apiKey: () => require_strings.hashSigningKey(this.signingKey),
			headers: () => this.headers,
			fetch: () => this.fetch
		});
		if (this.options.optimizeParallelism === false) require_log.warnOnce(this[internalLoggerSymbol], `optimize-parallelism-deprecated:${this.id}`, "`optimizeParallelism: false` is deprecated; use `group.parallel({ mode: \"race\" }, ...)` for race semantics instead");
		this.middleware = [...builtInMiddleware(this._logger), ...middleware ?? []];
		for (const mw of this.middleware) mw.onRegister?.({
			client: this,
			fn: null
		});
		if (this.aiMetadataEnabled) require_sdkFeatureObservations.sdkFeatureObservations.aiMetadata.replaceSetup(this, require_metadataProcessor.metadataSpanProcessor.attach());
	...
	*/
	setEnvVars(env = require_env.getProcessEnv()) {
		this._env = require_env.protectEnv({
			...this._env,
			...env
		});
		return this;
	...
	* @internal
	*/
	get [sessionPropagationSymbol]() {
		if (typeof this.options.sessionPropagation === "boolean") return this.options.sessionPropagation;
		const sessionPropagationEnv = require_env.parseAsBoolean(this._env[require_consts.envKeys.InngestSessionPropagation]);
		if (sessionPropagationEnv !== void 0) return sessionPropagationEnv;
	...
	*/
	async _send({ payload, headers, fn, fnMiddleware }) {
		...
		if (this[sessionPropagationSymbol]) {
			const asyncCtx = await require_als.getAsyncCtx();
			if (asyncCtx?.app === this) {
				const sessions = asyncCtx.execution?.ctx.sessions;
				if (sessions && Object.keys(sessions).length > 0) payloads = payloads.map((p) => require_sessions.stampPropagatedSessionsOnEvent(p, sessions, { onlyIfAbsent: true }));
			}
		}
		...
		payloads = payloads.map((p) => {
			const { sessions: _sessions, ctx: _ctx, ...rest } = p;
			return {
				...rest,
				id: p.id,
				ts: p.ts || nowMillis,
				data: p.data || {},
				meta: require_sessions.normalizeEventMeta(p.meta)
			};
		});
		/**
...
exports.sessionPropagationSymbol = sessionPropagationSymbol;
