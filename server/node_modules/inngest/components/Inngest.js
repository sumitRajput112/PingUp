import { defaultDevServerHost, defaultInngestApiBaseUrl, defaultInngestEventBaseUrl, dummyEventKey, envKeys, headerKeys } from "../helpers/consts.js";
import { getFetch, getProcessEnv, inngestHeaders, normalizeUrl, parseAsBoolean, protectEnv } from "../helpers/env.js";
import { fixEventKeyMissingSteps } from "../helpers/errors.js";
import { sendEventResponseSchema } from "../types.js";
import { formatLogMessage, warnOnce } from "../helpers/log.js";
import { hashSigningKey, stringify } from "../helpers/strings.js";
import { retryWithBackoff } from "../helpers/promises.js";
import { getAsyncCtx } from "./execution/als.js";
import { normalizeEventMeta, stampPropagatedSessionsOnEvent } from "../helpers/sessions.js";
import { InngestApi } from "../api/api.js";
import { createEntropy } from "../helpers/crypto.js";
import { ConsoleLogger, ProxyLogger } from "../middleware/logger.js";
import { metadataSpanProcessor } from "./execution/otel/metadataProcessor.js";
...
import { sendScore, sendScoreExperiment } from "./InngestScore.js";
import { step } from "./InngestStepTools.js";
import { buildWrapSendEventChain } from "./middleware/utils.js";
import { getSubscriptionToken, subscribe } from "./realtime/subscribe/index.js";
import { collectFeatureObservations, featureObservationStateSymbol, featureObservationsSymbol, sdkFeatureObservations } from "./sdkFeatureObservations.js";
import { isValidatable } from "./triggers/typeHelpers.js";
...
var Inngest = class Inngest {
	...
	*/
	...
	hasEventApiOriginOverride() {
		return Boolean(this.options.baseUrl || this._env[envKeys.InngestEventApiBaseUrl] || this._env[envKeys.InngestBaseUrl]);
	}
	...
	*/
	get score() {
		return Object.assign((options) => sendScore(this, options), { experiment: (options) => sendScoreExperiment(this, options) });
	}
	/**
	* A client used to interact with the Inngest API by sending or reacting to
	* events.
	...
	*/
	constructor(options) {
		...
		this[featureObservationStateSymbol] = sdkFeatureObservations.createState({ aiMetadataEnabled: this.aiMetadataEnabled });
		...
		if (this.aiMetadataEnabled) sdkFeatureObservations.aiMetadata.replaceSetup(this, metadataSpanProcessor.attach());
	...
	*/
	get [sessionPropagationSymbol]() {
		...
		const sessionPropagationEnv = parseAsBoolean(this._env[envKeys.InngestSessionPropagation]);
	...
	async _send({ payload, headers, fn, fnMiddleware }) {
		...
		if (this[sessionPropagationSymbol]) {
			...
			if (asyncCtx?.app === this) {
				...
				if (sessions && Object.keys(sessions).length > 0) payloads = payloads.map((p) => stampPropagatedSessionsOnEvent(p, sessions, { onlyIfAbsent: true }));
			}
		}
...
//#endregion
export { Inngest, internalLoggerSymbol, sessionPropagationSymbol };
